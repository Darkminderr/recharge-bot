...
async def process_payment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    async with async_playwright() as p:
        try:
            ...
            # 6. "PAGE EXPIRES IN" എന്ന ടൈമർ സ്ക്രീനിൽ വരുന്നതുവരെ കാത്തിരിക്കുന്നു (പരമാവധി 15 സെക്കൻഡ്)
            try:
                await page.wait_for_selector('text="PAGE EXPIRES IN"', timeout=15000)
            except:
                await asyncio.sleep(5) # എങ്കിലും ഒരു 5 സെക്കൻഡ് കാത്തിരിക്കുന്നു
            ...
            # 8. പെയ്‌മെന്റ് സക്സസ് ആകാൻ 8 മിനിറ്റ് കാത്തിരിക്കുന്നു (480,000 milliseconds)
            try:
                # നിങ്ങൾ തന്ന രണ്ടാമത്തെ ചിത്രത്തിലെ "Purchase successful" അല്ലെങ്കിൽ "Payment made successfully" എന്ന് വരുന്നതുവരെ നോക്കും
                ...
                # സക്സസ് ആയാൽ ഒരു സ്ക്രീൻഷോട്ട് കൂടി എടുക്കുന്നു
                await asyncio.sleep(2) # സക്സസ് പേജ് പൂർണ്ണമായി തെളിയാൻ
                await page.screenshot(path="success.png")
                
                # സക്സസ് സ്ക്രീൻഷോട്ടും മെസ്സേജും യൂസർക്ക് അയക്കുന്നു
                await update.message.reply_photo(
